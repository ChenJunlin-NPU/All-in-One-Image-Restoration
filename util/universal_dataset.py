...
class TrainDataset(Dataset):
    ...
    def _init_ids(self):
        if 'denoise' in self.de_type or 'denoise_15' in self.de_type or 'denoise_25' in self.de_type or 'denoise_50' in self.de_type:
    ...
    def _init_noise_clean_ids(self):
        ...
        if 'denoise' in self.de_type:
            self.noise_sigma_choices = [15, 25, 50]
            self.noise_ids = [
                {"clean_id": clean_id, "de_type": 0}
                for clean_id in clean_ids
            ]
            random.shuffle(self.noise_ids)
            self.noise_counter = 0
            self.num_clean = len(self.noise_ids)
            self.current_epoch = 0
            self.noise_sigma_schedule = []
            self.set_epoch(0)
        else:
            self.s15_ids = []
            self.s25_ids = []
            ...
            if 'denoise_15' in self.de_type:
                self.s15_ids = [{"clean_id": x, "de_type": 0, "noise_sigma": 15} for x in clean_ids]
                random.shuffle(self.s15_ids)
                self.s15_counter = 0
            if 'denoise_25' in self.de_type:
                self.s25_ids = [{"clean_id": x, "de_type": 1, "noise_sigma": 25} for x in clean_ids]
                random.shuffle(self.s25_ids)
                self.s25_counter = 0
            if 'denoise_50' in self.de_type:
                self.s50_ids = [{"clean_id": x, "de_type": 2, "noise_sigma": 50} for x in clean_ids]
    ...
    def _init_hazy_ids(self):
        # 直接从文件夹读取图片，不依赖txt文件
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.dehaze_dir, 'synthetic/'))
        temp_ids = image_list
        self.hazy_ids = [{"clean_id": x, "de_type": 4} for x in temp_ids]
        self.hazy_ids = self.hazy_ids * 10
        self.hazy_counter = 0
    ...
    def _init_rs_ids(self):
        # 直接从文件夹读取图片，不依赖txt文件
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.derain_dir, 'rainy/'))
        temp_ids = image_list
        self.rs_ids = [{"clean_id": x, "de_type": 3} for x in temp_ids]
        self.rs_ids = self.rs_ids * 25
    ...
    def _init_lowlight_ids(self):
        ...
        self.lowlight_ids = self.lowlight_ids * 10
    ...
    def _get_raingt_name(self, rainy_name):
        # 从rain-001.png生成norain-001.png
        gt_name = rainy_name.replace('rain-', 'norain-')
        return gt_name

    def _get_nonhazy_name(self, hazy_name):
        # 从dehaze-001_0.8_0.2.jpg生成dehaze-001.jpg
        name_parts = hazy_name.split('_')
        base_name = name_parts[0]  # dehaze-001
        suffix = '.' + hazy_name.split('.')[-1]  # .jpg
        nonhazy_name = base_name + suffix
        return nonhazy_name

    def _merge_ids(self):
        self.sample_ids = []
        if "denoise" in self.de_type:
            self.sample_ids += self.noise_ids
        else:
            if "denoise_15" in self.de_type:
    ...
    def set_epoch(self, epoch):
        if not (self.noise_combine and 'denoise' in self.de_type):
            return

        self.current_epoch = epoch
        sigma_schedule = self.noise_sigma_choices.copy()
        sigma_schedule *= (len(self.noise_ids) // len(self.noise_sigma_choices))
        sigma_schedule += self.noise_sigma_choices[:len(self.noise_ids) % len(self.noise_sigma_choices)]
        random.shuffle(sigma_schedule)
        self.noise_sigma_schedule = sigma_schedule
    ...
    def __getitem__(self, idx):
        ...
        if self.noise_combine and de_id == 0:
            clean_id = sample["clean_id"]
            noise_sigma = self.noise_sigma_schedule[idx % len(self.noise_sigma_schedule)]
            ...
            degrad_patch = self.D.single_degrade(clean_patch, noise_sigma=noise_sigma)
        elif de_id < 3:
            if de_id == 0:
                clean_id = sample["clean_id"]
            elif de_id == 1:
            ...
            if de_id == 3:
                ...
                clean_img = crop_img(np.array(
                    Image.open(os.path.join(self.args.derain_dir, 'gt/', self._get_raingt_name(sample["clean_id"]))).convert('RGB')),
                    base=16)
                clean_name = sample["clean_id"]
            elif de_id == 4:
                # Dehazing with SOTS outdoor training set
                degrad_img = crop_img(np.array(
                    Image.open(os.path.join(self.args.dehaze_dir, 'synthetic/', sample["clean_id"])).convert('RGB')),
                    base=16)
                clean_img = crop_img(np.array(
                    Image.open(os.path.join(self.args.dehaze_dir, 'original/', self._get_nonhazy_name(sample["clean_id"]))).convert('RGB')),
                    base=16)
                clean_name = sample["clean_id"]
            elif de_id == 5:
                #  debluring dataset
    ...
    def get_num_samples(self):
        ...
        if 'denoise' in self.de_type or 'denoise_25' in self.de_type or 'denoise_15' in self.de_type or 'denoise_50' in self.de_type:
            domain_sample_counts.append(self.num_clean)
        if 'derain' in self.de_type:
            domain_sample_counts.append(self.num_rl)
        if 'dehaze' in self.de_type:
            domain_sample_counts.append(self.num_hazy)
        if 'deblur' in self.de_type:
            domain_sample_counts.append(self.num_blur)
        if 'lowlight' in self.de_type:
            domain_sample_counts.append(self.num_lowlight)
        if 'single' in self.de_type:
            domain_sample_counts.append(self.num_single)

        return domain_sample_counts
# ================ The Testing Dataset ==================
# ================ The Testing Dataset ==================
# ================ The Testing Dataset ==================
# ================ The Testing Dataset ==================
